...
class HomeController extends Controller
{
    ...
    public function update(Request $request, $id){
        ...
        $texto->fill($datos);
        $texto->save();
        $success = 'texto editada correctamente';
        return back()->with('success', $success);
    }

    public function edit2(){
        $usuario = Auth::user();
        $texto = Texto::where('section', 'home2')->first();
        $home_seccion = 'active';
        $home_contenido_edit = 'active';

        return view('adm.home.editarContenido2',  compact('usuario', 'texto', 'home_contenido_edit', 'home_seccion'));
    }
